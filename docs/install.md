...
当前 mock 认证仍用于验证 Credential Provider 主链路，但 helper -> `auth_api` 已经具备短信发送、短信校验、二次密码校验和登录日志上报的真实 HTTP 请求代码，并通过 crate 内 mock 服务完成验证。默认占位服务地址下，短信和二次密码接口仍会回退到 mock 语义，登录日志接口会回退到本地 success fallback，便于在正式后端联调前继续验证登录流程。
